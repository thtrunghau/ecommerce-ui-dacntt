...
/**
 * Test function to demonstrate the best promotion selection with priority system
 * Priority: SPECIFIC_PRODUCTS > ALL_PRODUCTS
 * This function can be called from console or used for debugging
 */
export const testPromotionLogic = () => {
  console.log("=== TESTING PROMOTION LOGIC WITH PRIORITY SYSTEM ===");
  console.log("Priority: SPECIFIC_PRODUCTS > ALL_PRODUCTS\n");
  ...
  allPromotions.forEach((promo, index) => {
    console.log(
      `${index + 1}. ${promo.promotionName}: ${promo.discountAmount}${promo.isPercentage ? "%" : " VND"}`,
    );
  });
  ...
  if (priceInfo.hasActivePromotion && priceInfo.promotionInfo) {
    ...
    console.log(
      `Discount: ${priceInfo.promotionInfo.discountAmount}${priceInfo.promotionInfo.isPercentage ? "%" : " VND"}`,
    );
    console.log(
      `Final price: ${priceInfo.finalPrice.toLocaleString("vi-VN")} VND`,
    );
    console.log(
      `Savings: ${(priceInfo.originalPrice - priceInfo.finalPrice).toLocaleString("vi-VN")} VND`,
    );
  }
  ...
  const smartThingsProduct = mockProducts.find((p) =>
    p.productName.includes("SmartThings"),
  );
  if (smartThingsProduct) {
    console.log(`Testing product: ${smartThingsProduct.productName}`);
    console.log(
      `Original price: ${smartThingsProduct.price.toLocaleString("vi-VN")} VND`,
    );

    const allSmartPromotions = getAllApplicablePromotions(
      smartThingsProduct.id,
    );
    ...
    const smartPriceInfo = getProductPriceInfo(
      smartThingsProduct.id,
      smartThingsProduct.price,
    );
    if (smartPriceInfo.hasActivePromotion && smartPriceInfo.promotionInfo) {
      ...
      console.log(
        `Discount: ${smartPriceInfo.promotionInfo.discountAmount}${smartPriceInfo.promotionInfo.isPercentage ? "%" : " VND"}`,
      );
      console.log(
        `Final price: ${smartPriceInfo.finalPrice.toLocaleString("vi-VN")} VND`,
      );
      console.log(
        `Savings: ${(smartPriceInfo.originalPrice - smartPriceInfo.finalPrice).toLocaleString("vi-VN")} VND`,
      );
    }
  }
  console.log("\n" + "=".repeat(50));
  console.log(
    "✅ Test completed! SPECIFIC_PRODUCTS promotions are now prioritized over ALL_PRODUCTS.",
  );
};

// Export for use in console: testPromotionLogic()
// TypeScript-friendly way to add function to window for testing
declare global {
  interface Window {
    testPromotionLogic: () => void;
  }
}

if (typeof window !== "undefined") {
  window.testPromotionLogic = testPromotionLogic;
}
