...
const ErrorState: React.FC<{ message: string; onRetry?: () => void }> = ({
  ...
  <div className="flex flex-col items-center justify-center py-8">
    <div className="mb-2 text-lg font-semibold text-red-600">{message}</div>
    {onRetry && (
      <button
        onClick={onRetry}
        className="mt-2 rounded bg-black px-4 py-2 font-semibold text-white hover:bg-gray-800"
      >
        Thử lại
      </button>
    )}
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="flex justify-end gap-2">
        <button
          type="submit"
          className="rounded-full bg-black px-6 py-2 font-semibold text-white shadow transition hover:border hover:border-black hover:bg-white hover:text-black disabled:opacity-60"
          disabled={loading}
        >
          {initial ? "Lưu" : "Thêm"}
        </button>
        <button
          type="button"
          className="rounded-full border border-gray-400 px-6 py-2 font-semibold text-black shadow transition hover:bg-gray-100 disabled:opacity-60"
          onClick={onCancel}
          disabled={loading}
        >
          Hủy
...
const AdminCategories: React.FC = () => {
  ...
  const handleAdd = async (data: { categoryName: string }) => {
    ...
    try {
      ...
      await fetchCategories(); // Đảm bảo reload xong mới đóng form
      setShowForm(false);
    } catch (err: any) {
    ...
    <div className="mx-auto max-w-5xl px-2 py-8">
                  ...
                  <td className="py-2">
                    <div className="flex gap-2">
                      <button
                        ...
                        className="rounded-full border border-black bg-white px-4 py-1 font-semibold text-black shadow transition hover:bg-black hover:text-white disabled:opacity-60"
                        disabled={loading}
                      >
                        Sửa
                      </button>
                      <button
                        ...
                        className="rounded-full border border-red-500 bg-white px-4 py-1 font-semibold text-red-600 shadow transition hover:bg-red-500 hover:text-white disabled:opacity-60"
                        disabled={loading}
                      >
                        Xóa
                      </button>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="mx-2 w-full max-w-md rounded-xl bg-white p-6 shadow-xl sm:mx-auto">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="relative w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
            ...
            <div className="flex justify-end gap-2">
              <button
                onClick={() => setShowDelete(null)}
                className="rounded-full border border-gray-400 px-6 py-2 font-semibold text-black shadow transition hover:bg-gray-100"
                disabled={loading}
              >
                Hủy
              </button>
              <button
                ...
                className="rounded-full border border-red-500 bg-white px-6 py-2 font-semibold text-red-600 shadow transition hover:bg-red-500 hover:text-white"
                disabled={loading}
              >
                Xóa
              </button>
