...
import type { CategoryResDto, ProductResDto, PromotionResDto } from '../types/index';
...
export const mockPromotions: PromotionResDto[] = [
  ...
  },
  {
    ...
    productIds: mockProducts.filter(p => p.categoryId === mockCategories[1].id).map(p => p.id)
  },
  {
    id: uuidv4(),
    ...
    proportionType: "PERCENTAGE",
    minOrderValue: 5000000,
    productIds: mockProducts.filter(p => p.categoryId === mockCategories[5].id).map(p => p.id)
  }
];
