...
export const updateCartItem = async (cartItemId: string, quantity: number) => {
  ...
  try {
    if (quantity <= 0) {
      console.warn(`[API] Invalid quantity ${quantity}, removing item from cart instead`);
    ...
    console.log(`[API] Successfully updated cart item ${cartItemId} quantity to ${quantity}`);
    return extractData<CartItemResDto>(response);
  } catch (error) {
