...
import { GrantedAuthority } from '../types/api';
...
const tokenService = {
  ...
  getAuthorities(): string[] {
    ...
    try {
      ...
      if (decoded.authorities && Array.isArray(decoded.authorities)) {
        return decoded.authorities.map(auth => 
  ...
  hasAnyAuthority(requiredAuthorities: string[]): boolean {
    const authorities = this.getAuthorities();
    return requiredAuthorities.some(required => authorities.includes(required));
  },
  ...
  hasAllAuthorities(requiredAuthorities: string[]): boolean {
    const authorities = this.getAuthorities();
    return requiredAuthorities.every(required => authorities.includes(required));
  },
  ...
  isTokenExpiringSoon(): boolean {
    ...
    try {
      ...
      return decoded.exp < currentTime + fiveMinutes && decoded.exp > currentTime;
    } catch (error) {
      return false;
    }
  },
