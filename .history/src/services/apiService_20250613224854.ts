...
const buildUrl = (path: string): string => `${API_BASE_URL}${API_VERSION}${path}`;
...
export const productApi = {
  // GET /api/v1/products
  getList: async (params?: PaginationParams): Promise<ApiPageableResponse<ProductResDto>> => {
...
export const categoryApi = {
  ...
  getList: async (params?: PaginationParams): Promise<ApiPageableResponse<CategoryResDto>> => {
    const queryString = params ? buildQueryString(params) : '';
...
export const promotionApi = {
  ...
  getList: async (params?: PaginationParams): Promise<ApiPageableResponse<PromotionResDto>> => {
    const queryString = params ? buildQueryString(params) : '';
...
export const orderApi = {
  // GET /api/v1/orders
  getList: async (params?: PaginationParams): Promise<ApiPageableResponse<OrderResDto>> => {
    const queryString = params ? buildQueryString(params) : '';
  ...
  calculateTotalPrice: async (data: PlaceOrderReqDto): Promise<{ totalPrice: number }> => {
  ...
  takeAvailableCoupon: async (promotionIds: UUID[]): Promise<PromotionResDto[]> => {
...
export const addressApi = {
  ...
  getList: async (params?: PaginationParams): Promise<ApiPageableResponse<AddressResDto>> => {
    const queryString = params ? buildQueryString(params) : '';
  ...
  getByAccountId: async (accountId: UUID): Promise<AddressResDto[]> => {
    const response = await fetch(buildUrl(`/addresses/get-by-account/${accountId}`), {
      headers: getAuthHeaders(),
    });
...
export const accountApi = {
  ...
  signupWithGoogle: async (data: GoogleAuthRequestDTO): Promise<AccountResponseDTO> => {
  ...
  update: async (id: UUID, data: AccountRequestDTO): Promise<AccountResponseDTO> => {
    const response = await fetch(buildUrl(`/accounts/${id}`), {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
