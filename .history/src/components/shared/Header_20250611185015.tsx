            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {item.name}
                  </Link>                  {/* Invisible bridge for smooth hover experience - tăng chiều cao và mở rộng */}
                  {hasDropdown &&
                    activeDropdown ===
                      (isPromotion ? "promotion" : categoryId) && (
                      <div className="absolute left-[-20px] top-full h-12 w-[calc(100%+40px)] bg-transparent" />
                    )}
                </div>
              );
            })}
          ...
          (() => {
            ...
            if (!dropdownCategory) return null;            return (
              <div
                ...
                style={{ marginTop: '-12px' }} // Giảm khoảng cách để dropdown gần hơn và kết nối với bridge
              >
