    ...
    >
      ...
      <div className="container mx-auto px-4 py-2">
        <div className="relative flex items-center justify-between">
          ...
          >
            {navItems.map((item) => {
              // Chỉ show dropdown nếu có dữ liệu sản phẩm trong navDropdownData
              const dropdownData = navDropdownData.find(
                (cat) => cat.key === item.path.replace("/", ""),
              );
              const hasDropdown =
                item.name !== "Ưu Đãi" &&
                dropdownData &&
                dropdownData.products.length > 0;
              return (
                <div
                  key={item.id}
                  className="relative"
        ...
        {activeDropdown &&
          (() => {            const dropdownData = mockCategories.find(
              (cat) => cat.key === activeDropdown,
            );
            // Chỉ render dropdown nếu có sản phẩm
            if (!dropdownData || !dropdownData.products.length) return null;
            return (
              <div
              ...
              >
                <NavDropdown category={dropdownData} />
              </div>
            );
          })()}
      </div>
