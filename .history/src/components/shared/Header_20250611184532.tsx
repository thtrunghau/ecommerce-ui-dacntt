...
const Header: React.FC = () => {
  ...
  const handleMouseEnter = (dropdownId: string) => {
    ...
    }, 600); // Tăng lên 600ms để đảm bảo đủ thời gian di chuyển chuột
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {item.name}
                  </Link>                  {/* Invisible bridge for smooth hover experience - tăng chiều cao */}
                  {hasDropdown &&
                    activeDropdown ===
                      (isPromotion ? "promotion" : categoryId) && (
                      <div className="absolute left-[-10px] top-full h-8 w-[calc(100%+20px)] bg-transparent" />
                    )}
                </div>
              );
            })}
        ...
        {activeDropdown &&
          (() => {
            ...
            if (activeDropdown === "promotion") {
              // Lấy tất cả sản phẩm có trong promotion (productIds không rỗng)
            ...
            if (!dropdownCategory) return null;            return (
              <div
                onMouseEnter={handleDropdownEnter}
                onMouseLeave={handleDropdownLeave}
                className="dropdown-area absolute left-1/2 top-full z-50 -translate-x-1/2 transform"
                style={{ marginTop: '-4px' }} // Giảm khoảng cách giữa nav và dropdown
              >
