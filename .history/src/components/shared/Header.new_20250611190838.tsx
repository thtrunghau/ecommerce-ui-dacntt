...
const Header: React.FC = () => {
  ...
  const handleMouseLeave = () => {
    ...
    }, 300); // Giảm xuống 300ms để responsive hơn
  ...
  const handleDropdownLeave = () => {
    const timeout = setTimeout(() => {
      setActiveDropdown(null);
    }, 150); // Delay nhỏ để tránh mất dropdown khi di chuyển chuột nhanh
    setHoverTimeout(timeout);
  };

  // Helper function to get dropdown category
  const getDropdownCategory = (activeDropdown: string): CategoryResDto | null => {
    if (activeDropdown === "promotion") {
      ...
      const promoProductIds = activePromotions.flatMap(
        (promo) => promo.productIds,
      );
      const promoProducts = mockProducts.filter((product) =>
      ...
      return mockCategories.find((cat) => cat.id === activeDropdown) || null;
    }
  };
    ...
    >
      ...
      <div className="container mx-auto px-4 py-2">
        <div className="relative flex items-center justify-between">
          ...
          {/* Desktop Navigation với hover container */}
          <div className="mx-4 mt-[14px] hidden lg:flex relative">
            {/* Navigation items */}
            <nav className="flex">
              {navItems.map((item) => {
            ...
            {/* Dropdown container với vùng hover lớn */}
            {activeDropdown && (
              <div
                className="absolute left-0 top-full w-full z-50"
                onMouseLeave={handleMouseLeave}
                style={{ height: "60px" }} // Vùng hover lớn
              >
                ...
                {/* Dropdown content */}
                <div
                  className="absolute left-1/2 top-3 -translate-x-1/2 transform"
                  onMouseEnter={handleDropdownEnter}
                  onMouseLeave={handleDropdownLeave}
                >
                  {(() => {
                    const dropdownCategory = getDropdownCategory(activeDropdown);
