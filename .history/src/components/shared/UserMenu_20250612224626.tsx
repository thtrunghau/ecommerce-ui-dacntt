...
import { FiUser, FiLogIn, FiLogOut, FiUserPlus, FiShoppingBag, FiHeart, FiSettings } from 'react-icons/fi';
...
const UserMenu: React.FC<UserMenuProps> = ({ anchorEl, open, onClose }) => {
  ...
  return (
    <div 
      ref={menuRef}
      className="absolute z-50 min-w-[200px] rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none"
      ...
      <div className="py-1">
        {isAuthenticated ? (
          <>            <div className="px-4 py-3">
              ...
              {user?.phone && (
                <p className="text-xs text-gray-600 truncate mt-1">{user.phone}</p>
              )}
            </div>
            <hr />
            <Link 
