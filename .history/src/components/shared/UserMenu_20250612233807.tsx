...
const UserMenu: React.FC<UserMenuProps> = ({ anchorEl, open, onClose }) => {
  ...
  const getDropdownPosition = () => {
    ...
    // Position menu right below the avatar with minimal gap
    let top = rect.bottom + 4 + window.scrollY; // Minimal gap for clean look
    
    // Align menu to the right edge of avatar for better visual connection
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-40" onClick={onClose} />      <div
        ref={menuRef}
        className={`absolute z-50 min-w-[280px] transform rounded-2xl border border-gray-200/50 bg-white/95 shadow-2xl backdrop-blur-lg transition-all duration-300 ease-out focus:outline-none ${
          open
      ...
      >
        <div className="py-2 relative">
          ...
          <div 
            className="absolute -top-2 right-6 w-4 h-4 bg-white/95 border-l border-t border-gray-200/50 transform rotate-45"
            style={{
          ...
          {isAuthenticated ? (
            <>
              ...
              <div className="py-2">
                ...
                <Link
                  to="/wishlist"
                  className="group relative flex items-center overflow-hidden px-5 py-3.5 text-sm text-gray-700 transition-all duration-200 hover:bg-gradient-to-r hover:from-pink-50 hover:to-pink-100/50 hover:text-pink-700"
                  onClick={handleMenuItemClick}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-pink-500/0 to-pink-500/0 transition-all duration-300 group-hover:from-pink-500/5 group-hover:to-pink-500/10"></div>
                  <FiHeart className="mr-3 h-5 w-5 text-gray-400 transition-all duration-200 group-hover:scale-110 group-hover:text-pink-600" />
                  <span className="relative z-10 font-medium">
                    Danh sách yêu thích
                  </span>
                </Link>
