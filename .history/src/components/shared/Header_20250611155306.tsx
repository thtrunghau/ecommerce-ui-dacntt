...
const Header: React.FC = () => {
  ...
  const [activeDropdown, setActiveDropdown] = useState<string | null>(null); // Navigation items - exact font sizes and colors from Figma  const navItems = [
    {
      id: 1,
      name: "Ưu Đãi",
        ...
        <div className="relative flex items-center justify-between">
          ...
          >
            {navItems.map((item) => {
              ...
              const hasDropdown =
                dropdownData?.products && dropdownData.products.length > 0;
              return (
                <div
                  key={item.id}
                  className="relative"
                ...
                >
                  ...
                  {hasDropdown &&
                    activeDropdown === item.path.replace("/", "") && (
                      <div
                        className="absolute -bottom-5 h-5 w-full"
                        onMouseEnter={() =>
                          setActiveDropdown(item.path.replace("/", ""))
                        }
                      />
                    )}
                </div>
