...
const Header: React.FC = () => {
  ...
  const handleMouseEnter = (dropdownId: string) => {
    ...
    }, 500); // Tăng thời gian delay lên 500ms để user có đủ thời gian di chuyển
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {item.name}
                  </Link>                  {/* Invisible bridge for smooth hover experience */}
                  {hasDropdown &&
                    activeDropdown ===
                      (isPromotion ? "promotion" : categoryId) && (
                      <div className="absolute left-0 top-full h-6 w-full bg-transparent" />
                    )}
                </div>
              );
            })}
