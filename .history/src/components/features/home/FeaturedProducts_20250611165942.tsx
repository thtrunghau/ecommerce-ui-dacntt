  ...
  return (
    <Box sx={{ py: 6, bgcolor: "#f8f9fa" }}>
      <Container maxWidth="lg">
        ...
        {!isMobile ? (
          // Desktop Layout - 1 big featured + 4 grid          <Box sx={{ display: 'flex', flexWrap: 'wrap', margin: -1.5 }}>
            {/* Featured product - left side */}
            <Box sx={{ width: { xs: '100%', md: '41.66%' }, p: 1.5 }}>
              {featuredProduct && (
            ...
            <Box sx={{ width: { xs: '100%', md: '58.33%' }, p: 1.5 }}>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', margin: -1 }}>
                {gridProducts.map((product) => (
                  <Box sx={{ width: { xs: '50%' }, p: 1 }} key={product.id}>
